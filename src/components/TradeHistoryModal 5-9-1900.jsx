...
import { calculatePnlByInstrument, getInstrumentWithSettings } from '../instrumentHelpers';
...
function TradeHistoryModal({ trade, onClose, updateTrade, accounts = [], customStrategies = [], supabase, trades = [], userId }) {
  const [isEditing, setIsEditing] = useState(false);
  const [editedForm, setEditedForm] = useState(() => ({ ...trade }));
  const [instrumentSettings, setInstrumentSettings] = useState(null);

  // Sync with parent trades array when it updates (after save)
  useEffect(() => {
    const refreshedTrade = trades?.find(t => t.id === trade?.id);
    if (refreshedTrade && !isEditing) {
      setEditedForm({ ...refreshedTrade });
    }
  }, [trades, trade?.id, isEditing]);

  // Fetch instrument settings
  useEffect(() => {
    const fetchSettings = async () => {
      if (editedForm.symbol && supabase && userId && trade?.account_id) {
        const settings = await getInstrumentWithSettings(supabase, editedForm.symbol, trade.account_id, userId);
        setInstrumentSettings(settings);
      }
    };
    fetchSettings();
  }, [editedForm.symbol, supabase, userId, trade?.account_id]);

  const handleSave = async () => {
    try {
      // Recalculate P&L using instrument settings
      ...
      if (instrumentSettings) {
        ...
        newPnl = calculatePnlByInstrument(
          entry, exit, editedForm.direction, position,
          instrumentSettings.tickSize,
          instrumentSettings.tickValue
        );
      } else {
        // Fallback to hardcoded values (MNQ: $2 per point)
        const pointDiff = exit - entry;
        const dir = editedForm.direction === "Long" ? 1 : -1;
        newPnl = dir * pointDiff * 2 * position;
      }
      
      console.log('Modal P&L Calc:', {entry, exit, position, newPnl}); // DEBUG
      ...
      const result = await updateTrade(trade.id, updatedForm);
      ...
      // Close modal after brief delay to show the update
      setTimeout(() => {
        onClose();
      }, 500);
    } catch (error) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {!isEditing ? (
          <TradeInfoReadOnly trade={editedForm} onEdit={() => setIsEditing(true)} />
        ) : (
          <TradeEditForm 
...
function TradeEditForm({ editedForm, setEditedForm, onSave, onCancel, accounts, customStrategies, instrumentSettings }) {
  const builtInList = [
  ...
  const estPnl = editedForm.entry_price && editedForm.exit_price && editedForm.position_size
    ? (() => {
        ...
        if (instrumentSettings) {
          return calculatePnlByInstrument(
            entry, exit, editedForm.direction, position,
            instrumentSettings.tickSize,
            instrumentSettings.tickValue
          );
        ...
        // Fallback: MNQ $2 per point
        const pointDiff = exit - entry;
        const dir = editedForm.direction === "Long" ? 1 : -1;
        return dir * pointDiff * 2 * position;
      })()
    : 0;
