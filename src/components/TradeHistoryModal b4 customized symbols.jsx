...
function TradeHistoryModal({ trade, onClose, updateTrade, accounts = [], customStrategies = [], supabase, trades = [] }) {
  ...
  const handleSave = async () => {
    try {
      // Recalculate P&L: MNQ uses $2 per point multiplier
      ...
      const newPnl = dir * pointDiff * 2 * position;
      
      console.log('Modal P&L Calc:', {entry, exit, position, pointDiff, dir, newPnl}); // DEBUG
      
      // Determine result based on new P&L (commissions/fees are separate)
...
function TradeEditForm({ editedForm, setEditedForm, onSave, onCancel, accounts, customStrategies }) {
  ...
  const estPnl = editedForm.entry_price && editedForm.exit_price && editedForm.position_size
    ? (() => {
        ...
        const pointDiff = exit - entry;
        const dir = editedForm.direction === "Long" ? 1 : -1;
        // MNQ: $2 per point multiplier (commissions/fees shown separately)
