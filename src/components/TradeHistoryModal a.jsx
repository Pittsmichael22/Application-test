    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          {!isEditMode ? (
            <TradeInfoReadOnly trade={trade} onEdit={() => setIsEditMode(true)} />
          ) : (
            <TradeEditForm 
...
// =========================================================================
// READ-ONLY VIEW - Shows All Entered Data
// =========================================================================
    ...
    <div>
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, fontSize: 13 }}>
          ...
          <div><strong>Duration:</strong> {formatValue(trade.duration_minutes)} min</div>
        </div>
      </section>
      ...
      <section>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <p style={{ whiteSpace: "pre-wrap" }}>{formatValue(trade.post_trade_reflection)}</p>
        </div>

        <div style={{ marginTop: 16 }}>
          <strong>What would I do differently next time?</strong>
          <p style={{ whiteSpace: "pre-wrap" }}>{formatValue(trade.what_to_improve)}</p>
        </div>
      </section>
...
// =========================================================================
// FULL EDIT FORM - MATCHES ALL 3 STAGES
// =========================================================================
    ...
    <div>
      ...
      {accounts.length > 0 && (
        <div style={{marginBottom: 20}}>
          <label style={{ display: "block", marginBottom: 8, color: "#ffffff", fontWeight: 600 }}>Account</label>
          <select 
            value={editedForm.account_id || ""} 
            onChange={e => setEditedForm({...editedForm, account_id: e.target.value})}
            style={{ width: "100%", padding: 14, background: "#1a1d2e", color: "#ffffff", border: `1px solid #1c1f30`, borderRadius: 8 }}
          >
            {accounts.map(acc => (
      ...
      <div style={{marginBottom: 28}}>
        ...
        <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16}}>
          ...
          <div>
            ...
            <select value={editedForm.setup_type || ''} onChange={e => setEditedForm({...editedForm, setup_type: e.target.value})}>
              <option value="">Select...</option>
              {builtInList.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
              {customStrategies.map(strat => <option key={strat.id} value={strat.name}>{strat.name}</option>)}
            </select>
          ...
          <div>
            ...
            <input type="text" value={editedForm.entry_time || ''} onChange={e => setEditedForm({...editedForm, entry_time: e.target.value})} />
          </div>
          <div>
            <label>Exit Time</label>
            <input type="text" value={editedForm.exit_time || ''} onChange={e => setEditedForm({...editedForm, exit_time: e.target.value})} />
          </div>
        </div>
      </div>
