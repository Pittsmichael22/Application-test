...
function Analytics({ trades }) {
  ...
  return (
    <div>
      ...
      {tab === "setups" && (
        <Card>
          <SectionTitle>Performance by Setup Type</SectionTitle>
          {analytics.setupData.map(d => (
            <div key={d.setup} style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", borderTop: "1px solid #1c1f30" }}>
              <span>{d.setup}</span>
              <span>{d.winRate}% WR • {d.total} trades • {fmt$(d.pnl)}</span>
            </div>
          ))}
        </Card>
      )}
