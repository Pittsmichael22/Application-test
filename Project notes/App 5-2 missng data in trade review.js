            ...
            <div>
              ...
              <input 
                ...
                onChange={e => {
                  ...
                  // Auto-format as you type: HH:MM:SS.mmm
                  if (val.length > 0) {
                    if (val.length <= 2) {
                      val = val; // HH
                    } else if (val.length <= 4) {
                      val = val.slice(0, 2) + ':' + val.slice(2); // HH:MM
                    } else if (val.length <= 6) {
                      val = val.slice(0, 2) + ':' + val.slice(2, 4) + ':' + val.slice(4); // HH:MM:SS
                    } else if (val.length <= 9) {
                      val = val.slice(0, 2) + ':' + val.slice(2, 4) + ':' + val.slice(4, 6) + '.' + val.slice(6); // HH:MM:SS.mmm
                    } else {
                      val = val.slice(0, 2) + ':' + val.slice(2, 4) + ':' + val.slice(4, 6) + '.' + val.slice(6, 9); // Limit to mmm
                    }
                  }
