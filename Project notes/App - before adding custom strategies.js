...
function TradeLog({ trades, deleteTrade, updateTrade, setView, openTradeReview }) {
  ...
  const startEdit = (trade) => {
    ...
    sessionStorage.setItem('editingStage', 'pre-trade');  
    setView('entry');
  };
                ...
                >
                  ...
                  <td style={{ padding: "11px 14px", display: "flex", gap: 6 }}>
                    <button onClick={(e) => { e.stopPropagation(); startEdit(t); }}
                      ...
                      onMouseLeave={e => e.currentTarget.style.color = C.muted}>✎</button>
                    <button onClick={(e) => { e.stopPropagation(); deleteTrade_local(t.id); }}
                      style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 14, padding: "2px 6px", borderRadius: 4 }}
                      onMouseEnter={e => e.currentTarget.style.color = C.red}
                      onMouseLeave={e => e.currentTarget.style.color = C.muted}>✕</button>
                  </td>
...
export default function App() {
 ...
 const userId = session?.user?.id || null;
  ...
  // REVIEW MODAL STATE + FUNCTIONS (CORRECT PLACE)
