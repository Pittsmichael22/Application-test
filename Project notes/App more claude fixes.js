  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'DM Sans', -apple-system, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ 
        ...
        <div style={{ fontSize: 18, fontWeight: 800, background: `linear-gradient(135deg, ${C.text}, ${C.sub})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
          futures.os
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ 
      ...
      <div style={{
        ...
        {NAV.map(item => (
          <button
            ...
            onMouseEnter={(e) => e.currentTarget.style.color = view === item.id ? C.blue : C.text}
            onMouseLeave={(e) => e.currentTarget.style.color = view === item.id ? C.blue : C.muted}
          >
            <div style={{ fontSize: 20 }}>{item.emoji}</div>
            <div style={{ fontSize: 9, lineHeight: "1.2" }}>{item.label}</div>
          </button>
        ))}
      </div>
