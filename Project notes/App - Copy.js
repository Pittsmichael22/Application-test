...
// ─────────────────────────────────────────────────────────────
// FULL TRADE ENTRY - 3 STAGE (UPDATED)
// ─────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────
// FULL TRADE ENTRY - COMPLETE 3-STAGE VERSION
// ─────────────────────────────────────────────────────────────
function TradeEntry({ addTrade, updateTrade, setView, trades }) {
  ...
  const canProceed = () => {
    ...
    if (stage === "trade-entry") return form.entry_price && form.exit_price;
    if (stage === "post-trade") return true; // Reflection is optional
    return false;
  };
  ...
  const handleNext = async () => {
    ...
    if (stage === "pre-trade") setStage("trade-entry");
      ...
      const finalTrade = { 
        ...
        ...postTradeForm, 
        pnl: estPnl,
        direction: parseFloat(form.entry_price) < parseFloat(form.exit_price) ? "Long" : "Short",
  ...
  return (
   ...
   {/* STAGE 1: PRE-TRADE */}
      {stage === "pre-trade" && (
        <Card>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#ffffff", display: "block", marginBottom: 10 }}>Market Condition</label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {[
                { id: "trending_up", label: "TRENDING UP", color: C.green },
                { id: "trending_down", label: "TRENDING DOWN", color: C.red },
                { id: "ranging", label: "RANGING", color: C.blue },
                { id: "volatile", label: "VOLATILE", color: C.purple }
              ].map(opt => (
                <button key={opt.id} onClick={() => setForm(p => ({...p, market_condition: opt.id}))}
                  style={{
                    padding: "16px", borderRadius: 10, fontWeight: 700,
                    border: `2px solid ${form.market_condition === opt.id ? opt.color : C.border}`,
                    background: form.market_condition === opt.id ? opt.color + "20" : "#1a1d2e",
                    color: form.market_condition === opt.id ? opt.color : "#ffffff"
                  }}>
                  {opt.label}
                </button>
              ))}
          ...
          {/* Entry Signal */}
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#ffffff", display: "block", marginBottom: 8 }}>Entry Signal</label>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={{ color: "#ffffff" }}>Conviction Level: {form.conviction_level}/10</label>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, color: "#ffffff" }}>
            <input type="checkbox" checked={form.is_news_event} onChange={e => setForm(p => ({...p, is_news_event: e.target.checked}))} />
            Trade during or around a significant news event
          </label>

          {/* Mindset */}
          <div style={{ marginTop: 24, padding: 16, background: "#1a1d2e", borderRadius: 8 }}>
            <h4 style={{ color: "#ffffff", marginBottom: 16 }}>Mindset</h4>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", marginBottom: 8, color: "#ffffff" }}>Mental State</label>
              <select value={form.mental_state || ""} onChange={e => setForm(p => ({...p, mental_state: e.target.value}))} style={{ width: "100%", padding: 12, background: "#1a1d2e", color: "#e0e0e0", borderRadius: 8 }}>
            ...
            <div>
              <label style={{ display: "block", marginBottom: 8, color: "#ffffff" }}>Emotional Intensity (1-10)</label>
              <input type="range" min="1" max="10" value={form.emotional_intensity || 5} onChange={e => setForm(p => ({...p, emotional_intensity: +e.target.value}))} style={{ width: "100%", accentColor: C.blue }} />
            </div>
          </div>

          <Btn onClick={handleNext} disabled={!canProceed()} style={{ width: "100%", marginTop: 24 }}>Next → Execution</Btn>
        </Card>
      )}
      ...
      {/* STAGE 2: EXECUTION - POSITION SIZE WITH TRADE DATE */}
      {stage === "trade-entry" && (
        <Card>
          <h3>2. Execution</h3>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
            <div>
              ...
              <input type="time" value={form.entry_time || ""} onChange={e => setForm(p => ({...p, entry_time: e.target.value}))} style={{ width: "100%", padding: 14, background: "#1a1d2e", color: "#e0e0e0", borderRadius: 8 }} />
            </div>
            <div>
              <label style={{ display: "block", marginBottom: 8, color: "#ffffff" }}>Exit Time</label>
              <input type="time" value={form.exit_time || ""} onChange={e => setForm(p => ({...p, exit_time: e.target.value}))} style={{ width: "100%", padding: 14, background: "#1a1d2e", color: "#e0e0e0", borderRadius: 8 }} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
            <div>
              ...
              <input type="number" step="0.01" value={form.entry_price} onChange={e => setForm(p => ({...p, entry_price: e.target.value}))} style={{ width: "100%", padding: 16, background: "#1a1d2e", color: "#ffffff", borderRadius: 8 }} />
            </div>
            <div>
              <label style={{ display: "block", marginBottom: 8, color: "#ffffff" }}>Exit Price</label>
              <input type="number" step="0.01" value={form.exit_price} onChange={e => setForm(p => ({...p, exit_price: e.target.value}))} style={{ width: "100%", padding: 16, background: "#1a1d2e", color: "#ffffff", borderRadius: 8 }} />
            </div>
          </div>
      ...
      {stage === "post-trade" && (
        <Card>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={{ display: "block", marginBottom: 12, color: "#ffffff" }}>Any trade rule violations?</label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                ...
                <label key={rule} style={{ display: "flex", alignItems: "center", gap: 8, color: "#e0e0e0", cursor: "pointer" }}>
                  <input 
                    type="checkbox" 
                    checked={postTradeForm.rule_violations.includes(rule)}
                    onChange={() => {
                      const newViolations = postTradeForm.rule_violations.includes(rule)
                        ? postTradeForm.rule_violations.filter(v => v !== rule)
                        : [...postTradeForm.rule_violations, rule];
                      setPostTradeForm(p => ({...p, rule_violations: newViolations}));
                    }}
                  />
                  {rule}
                </label>
          ...
          <textarea 
            placeholder="What did I learn from this trade? (optional)" 
            value={postTradeForm.post_trade_reflection} 
            onChange={e => setPostTradeForm(p => ({...p, post_trade_reflection: e.target.value}))} 
            style={{ width: "100%", minHeight: 140, padding: 14, background: "#1a1d2e", color: "#ffffff", borderRadius: 8 }}
          />
