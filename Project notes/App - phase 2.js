...
// Function to check if token is expired and refresh it
...
const addTrade = useCallback(async (trade, overrideDate = null, overrideTime = null) => {
  ...
  return { trades, loading, addTrade, deleteTrade, updateTrade, refetch: fetchTrades, isConfigured };
}
...
// ─────────────────────────────────────────────────────────────
// TRADE ENTRY (with CSV import)
// ─────────────────────────────────────────────────────────────
function TradeEntry({ addTrade, updateTrade, setView, trades }) {
  const [tab, setTab] = useState("manual"); // manual | csv
  const [editingTradeId, setEditingTradeId] = useState(null);
  const [stage, setStage] = useState("pre-trade"); // "pre-trade" | "trade-entry" | "post-trade"
  ...
  const [form, setForm] = useState({ 
    ...
    emotional_intensity: "", 
    trade_behaviors: [],
    // 🆕 NEW BEHAVIOR ANALYTICS FIELDS
    decision_speed: "moderate",
    conviction_level: 5,
    entry_signal: "",
    market_condition: "",
    pre_trade_checklist_passed: false,
    is_news_event: false,
  });
  ...
  useEffect(() => {
    const tradeId = sessionStorage.getItem('editingTradeId');
    const tradeData = sessionStorage.getItem('editingTradeData');
    const editingStage = sessionStorage.getItem('editingStage');  // 🆕 Read editing stage
    if (tradeId && tradeData) {
      try {
        ...
        setForm({
          ...
          fees: trade.fees || "",
          entry_time: trade.trade_time || trade.entry_time || "",
          position_size: trade.position_size || "1",
          stop_loss: trade.stop_loss || "",
          take_profit: trade.take_profit || "",
          setup_type: trade.setup_type || "A+",
          notes: trade.notes || "",
          mental_state: trade.mental_state || "",
          emotional_intensity: trade.emotional_intensity || "",
          trade_behaviors: trade.trade_behaviors || [],
          // 🆕 Load behavior analytics fields
          decision_speed: trade.decision_speed || "moderate",
          conviction_level: trade.conviction_level || 5,
          entry_signal: trade.entry_signal || "",
          market_condition: trade.market_condition || "",
          pre_trade_checklist_passed: trade.pre_trade_checklist_passed || false,
          is_news_event: trade.is_news_event || false,
        });
        setPostTradeForm({
          exit_reason: trade.exit_reason || "",
          fear_level: trade.fear_level || 5,
          greed_level: trade.greed_level || 5,
          rule_violations: trade.rule_violations || [],
          post_trade_reflection: trade.post_trade_reflection || "",
        });
        // 🆕 Reset stage to pre-trade for full editing
        if (editingStage) {
          setStage(editingStage);
        } else {
          setStage('pre-trade');  // Default to Stage 1
        }
      } catch (e) {
        console.error("Error parsing editing trade data:", e);
      }
    }
  }, []);

  const calcPnl = () => {
    const e = parseFloat(form.entry_price), x = parseFloat(form.exit_price), s = parseFloat(form.position_size) || 1;
    if (!e || !x) return null;
    const tickMap = {
      NQ: { tick: 0.25, value: 5 },    MNQ: { tick: 0.25, value: 0.50 },
      ES: { tick: 0.25, value: 12.5 }, MES: { tick: 0.25, value: 1.25 },
      RTY: { tick: 0.10, value: 5 },   M2K: { tick: 0.10, value: 0.50 },
      YM: { tick: 1,    value: 5 },    MYM: { tick: 1,    value: 0.50 },
      CL: { tick: 0.01, value: 10 },   MCL: { tick: 0.01, value: 1.00 },
      GC: { tick: 0.10, value: 10 },   MGC: { tick: 0.10, value: 1.00 },
      SI: { tick: 0.005,value: 25 },   ZB: { tick: 0.03125, value: 31.25 },
      ZN: { tick: 0.015625, value: 15.625 }, ZF: { tick: 0.0078125, value: 7.8125 },
    };
    const spec = tickMap[form.symbol] || { tick: 0.25, value: 5 };
    const ticks = (x - e) / spec.tick;
    const dir = form.direction === "Long" ? 1 : -1;
    return dir * ticks * spec.value * s;
  };
...
const handlePreTradeSubmit = () => {
  ...
  if (!form.market_condition || !form.entry_signal || !form.conviction_level) {
    alert("Complete the pre-trade checklist");
    return;
  }
  // Move to trade entry stage
  setStage("trade-entry");
};

// Stage 2: TRADE ENTRY (existing logic, now in middle)
const handleTradeEntryNext = () => {
  if (!form.entry_price || !form.exit_price) {
    alert("Enter entry and exit prices");
    return;
  }
  // Move to post-trade stage
  setStage("post-trade");
};

// Stage 3: POST-TRADE reflection + final submit
const handlePostTradeSubmit = async () => {
  if (!postTradeForm.exit_reason || !postTradeForm.post_trade_reflection) {
    alert("Complete post-trade reflection");
    return;
  }
  ...
  const exit_price = parseFloat(form.exit_price);
  const pnl = estPnl;
  
  // ✅ COMPUTE direction based on entry vs exit price
  const direction = entry_price < exit_price ? "Long" : "Short";
  
  // ✅ COMPUTE result based on PnL
  const result = pnl > 0 ? "Win" : pnl < 0 ? "Loss" : "Breakeven";
  
  // ✅ USE setup_type from form (with fallback)
  const setup_type = form.setup_type || "A+";
  
  const trade = { 
    ...form,
    ...postTradeForm,  // 🆕 Merge post-trade data
    entry_price,
    exit_price,
  ...
  if (editingTradeId && form.id) {
    // EDIT mode: trade already has an ID
    await updateTrade(form.id, trade);
    // Clear editing session
    sessionStorage.removeItem('editingTradeId');
    sessionStorage.removeItem('editingTradeData');
    sessionStorage.removeItem('editingStage');  // 🆕 Also clear stage
  } else {
    // CREATE mode: no ID, new trade
    await addTrade(trade);
  }
  
  setSubmitting(false);
  setDone(true);
  setTimeout(() => setView("journal"), 600);
};
const handleCsvImport = async () => {
    if (!csvTrades.length) return;
    setSubmitting(true);
    
    // UPDATED: Ensure direction, result, and setup_type are computed for each CSV trade
    for (const t of csvTrades) {
      // Parse numeric values
      const entry_price = parseFloat(t.entry_price);
      const exit_price = parseFloat(t.exit_price);
      ...
      // ✅ USE setup_type from CSV or default to "A+"
      const setup_type = t.setup_type || "A+";
      
      // ✅ FIX date format to ISO (YYYY-MM-DD)
      const trade_date = t.trade_date 
      ...
      await addTrade(tradeWithTime);
    }
    
    setSubmitting(false);
    setDone(true);
    setTimeout(() => setView("analytics"), 800);
  };
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!parsed.length) { setCsvError("No trades found. Check your CSV format."); return; }
        setCsvTrades(parsed);
      } catch { setCsvError("Failed to parse CSV. Please check the file format."); }
    };
  ...
  const inp = (label, key, placeholder, type = "text") => (
    <div>
      ...
      <input type={type} value={form[key]} placeholder={placeholder}
        ...
        style={{ width: "100%", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontSize: 13, outline: "none", boxSizing: "border-box", fontFamily: "monospace" }} />
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 620, margin: "0 auto" }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <h2 style={{ margin: "8px 0 4px", fontSize: 20, fontWeight: 800 }}>Log Trade</h2>
        <p style={{ color: C.muted, fontSize: 12, margin: 0 }}>Manual entry or broker CSV import</p>
      </div>

      {/* Tab Switch */}
      <div style={{ display: "flex", gap: 4, background: "#0b0d19", border: `1px solid ${C.border}`, borderRadius: 10, padding: 4, marginBottom: 20 }}>
        {[["manual", "✍️ Manual Entry"], ["csv", "📁 CSV Import"]].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{ flex: 1, padding: "9px", borderRadius: 7, border: "none", background: tab === id ? C.panel : "transparent", color: tab === id ? C.text : C.muted, cursor: "pointer", fontSize: 13, fontWeight: tab === id ? 700 : 400, transition: "all 0.15s" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === "manual" ? (
        <Card>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              {[
                { id: "pre-trade", label: "1. Pre-Trade", icon: "📋" },
                { id: "trade-entry", label: "2. Trade Entry", icon: "✍️" },
                { id: "post-trade", label: "3. Post-Trade", icon: "🔍" }
              ].map((s) => (
                <div key={s.id} style={{
                  flex: 1,
                  padding: "8px 12px",
                  borderRadius: 6,
                  background: stage === s.id ? C.blue + "20" : "#0b0d19",
                  border: `1px solid ${stage === s.id ? C.blue : C.border}20`,
                  textAlign: "center",
                  fontSize: 11,
                  fontWeight: stage === s.id ? 700 : 400,
                  color: stage === s.id ? C.blue : C.muted,
                }}>
                  {s.icon} {s.label}
                </div>
              ))}
            ...
            {stage === "pre-trade" && (
              <>
                <div style={{ background: "#0b0d19", border: `1px solid ${C.border}`, borderRadius: 10, padding: 16 }}>
                  <h3 style={{ fontSize: 13, fontWeight: 800, color: C.text, marginBottom: 16 }}>📋 Pre-Trade Checklist</h3>
                  
                  {/* Market Condition */}
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>What's the market condition?</label>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {[
                        { id: "trending_up", label: "📈 Trending Up", color: C.green },
                        { id: "trending_down", label: "📉 Trending Down", color: C.red },
                        { id: "ranging", label: "↔️ Ranging", color: C.blue },
                        { id: "volatile", label: "⚡ Volatile", color: C.purple }
                      ].map(opt => (
                        <button key={opt.id}
                          onClick={() => setForm({...form, market_condition: opt.id})}
                          style={{
                            padding: "10px",
                            borderRadius: 8,
                            border: `1px solid ${form.market_condition === opt.id ? opt.color : C.border}`,
                            background: form.market_condition === opt.id ? opt.color + "18" : "transparent",
                            color: form.market_condition === opt.id ? opt.color : C.muted,
                            cursor: "pointer",
                            fontSize: 12,
                            fontWeight: form.market_condition === opt.id ? 700 : 400
                          }}>
                          {opt.label}
                        </button>
                      ))}
                  ...
                  {/* Entry Signal */}
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>Entry signal type?</label>
                    <select value={form.entry_signal || ""} onChange={e => setForm({...form, entry_signal: e.target.value})}
                      style={{ width: "100%", padding: "10px", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12, cursor: "pointer" }}>
                      ...
                      <option value="trendline_break">Trendline Break</option>
                  ...
                  {/* Conviction Level */}
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>
                      Conviction Level: <span style={{ color: C.blue, fontWeight: 800 }}>{form.conviction_level}/10</span>
                    </label>
                    <input type="range" min="1" max="10" value={form.conviction_level}
                      onChange={e => setForm({...form, conviction_level: parseInt(e.target.value)})}
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 12 }}>
                      <input type="checkbox" checked={form.is_news_event}
                        onChange={e => setForm({...form, is_news_event: e.target.checked})}
                        style={{ cursor: "pointer", width: 16, height: 16 }} />
                      <span>Trade during or around a significant news event?</span>
                    </label>
                  </div>

                  {/* Decision Speed */}
                  <div>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>Decision Speed?</label>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                      {[
                        { id: "fast", label: "⚡ Fast" },
                        { id: "moderate", label: "➡️ Moderate" },
                        { id: "overthink", label: "🤔 Overthink" }
                      ].map(opt => (
                        <button key={opt.id}
                          onClick={() => setForm({...form, decision_speed: opt.id})}
                          style={{
                            padding: "8px",
                            borderRadius: 6,
                            border: `1px solid ${form.decision_speed === opt.id ? C.blue : C.border}`,
                            background: form.decision_speed === opt.id ? C.blue + "18" : "transparent",
                            color: form.decision_speed === opt.id ? C.blue : C.muted,
                            cursor: "pointer",
                            fontSize: 11,
                            fontWeight: form.decision_speed === opt.id ? 700 : 400
                          }}>
                          {opt.label}
                        </button>
                      ))}
                ...
                <Btn onClick={handlePreTradeSubmit} style={{ width: "100%", padding: 14, fontSize: 14, background: C.blue }}>
                  ✓ Pre-Trade Complete → Next Step
                </Btn>
              </>
            )}
              ...
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    <div>
                      <label style={{ fontSize: 10, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.1em" }}>Symbol</label>
                      <SymbolInput form={form} setForm={setForm} trades={trades} />
                    </div>
                    <div>
                      <label style={{ fontSize: 10, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.1em" }}>Direction</label>
                      <div style={{ display: "flex", gap: 8 }}>
                        {["Long", "Short"].map(d => (
                          <button key={d} onClick={() => setForm(p => ({ ...p, direction: d }))}
                            style={{ flex: 1, padding: "10px", borderRadius: 8, border: `1px solid ${form.direction === d ? (d === "Long" ? C.green : C.red) : C.border}`, background: form.direction === d ? (d === "Long" ? C.green + "18" : C.red + "18") : "transparent", color: form.direction === d ? (d === "Long" ? C.green : C.red) : C.muted, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
                            {d === "Long" ? "↑ Long" : "↓ Short"}
                          </button>
                        ))}
                      </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    {inp("Entry Price", "entry_price", "e.g. 21450.25", "number")}
                    {inp("Exit Price", "exit_price", "e.g. 21487.50", "number")}
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
                    {inp("Size (contracts)", "position_size", "1", "number")}
                    {inp("Stop Loss", "stop_loss", "optional", "number")}
                    {inp("Take Profit", "take_profit", "optional", "number")}
                  </div>

                  {/* Est PnL */}
                  {estPnl !== null && (
                    <div style={{ background: (estPnl >= 0 ? C.green : C.red) + "10", border: `1px solid ${(estPnl >= 0 ? C.green : C.red)}25`, borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: 12, color: C.muted }}>Estimated P&L</span>
                      <span style={{ fontSize: 18, fontWeight: 800, color: estPnl >= 0 ? C.green : C.red, fontFamily: "monospace" }}>{fmt$(estPnl)}</span>
                    </div>
                  )}

                  {/* Setup Type */}
                  <div>
                    <label style={{ fontSize: 10, color: C.muted, display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.1em" }}>Setup Type</label>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {["A+", "A", "B", "C", "Custom"].map(s => (
                        <button key={s} onClick={() => setForm(p => ({ ...p, setup_type: s }))}
                          style={{ padding: "7px 16px", borderRadius: 20, border: `1px solid ${form.setup_type === s ? C.purple : C.border}`, background: form.setup_type === s ? C.purple + "20" : "transparent", color: form.setup_type === s ? C.purple : C.muted, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                          {s}
                        </button>
                      ))}
                    </div>
                  ...
                  {/* Mental State + Intensity (from old form) */}
                  <div style={{ background: "#0b0d19", border: `1px solid ${C.border}`, borderRadius: 10, padding: 12 }}>
                    <div style={{ fontSize: 10, color: C.muted, marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.1em" }}>Entry Mindset</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      <div>
                        <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 6, fontWeight: 600 }}>Mental State</label>
                        <select value={form.mental_state || ""} onChange={e => setForm({...form, mental_state: e.target.value})}
                          style={{ width: "100%", padding: "8px 10px", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12, cursor: "pointer" }}>
                      ...
                      <div>
                        <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 6, fontWeight: 600 }}>Intensity</label>
                        <select value={form.emotional_intensity || ""} onChange={e => setForm({...form, emotional_intensity: e.target.value})}
                          style={{ width: "100%", padding: "8px 10px", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12, cursor: "pointer" }}>
                          <option value="">Select...</option>
                  ...
                  <div>
                    <label style={{ fontSize: 10, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.1em" }}>Notes (optional)</label>
                    <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="What did you observe?"
                      style={{ width: "100%", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontSize: 13, outline: "none", boxSizing: "border-box", resize: "vertical", minHeight: 60, fontFamily: "inherit" }} />
                  </div>
                </div>

                <div style={{ display: "flex", gap: 8 }}>
                  <Btn onClick={() => setStage("pre-trade")} variant="secondary" style={{ flex: 1, padding: 12, fontSize: 13 }}>
                    ← Back
                  </Btn>
                  <Btn onClick={handleTradeEntryNext} disabled={!form.entry_price || !form.exit_price} style={{ flex: 1, padding: 12, fontSize: 13 }}>
                    Next → Post-Trade
                  </Btn>
                </div>
              </>
            ...
            {/* STAGE 3: POST-TRADE REFLECTION */}
            {stage === "post-trade" && (
              <>
                <div style={{ background: "#0b0d19", border: `1px solid ${C.border}`, borderRadius: 10, padding: 16 }}>
                  <h3 style={{ fontSize: 13, fontWeight: 800, color: C.text, marginBottom: 16 }}>🔍 Post-Trade Reflection</h3>

                  {/* Exit Reason */}
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>Why did you exit?</label>
                    <select value={postTradeForm.exit_reason || ""} onChange={e => setPostTradeForm({...postTradeForm, exit_reason: e.target.value})}
                      style={{ width: "100%", padding: "10px", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12, cursor: "pointer" }}>
                      <option value="">Select reason...</option>
                      <option value="target_hit">✓ Target Hit</option>
                      <option value="stop_hit">⛔ Stop Loss Hit</option>
                      <option value="manual_exit">🛑 Manual Exit</option>
                      <option value="abandoned">❌ Abandoned (gave up)</option>
                    </select>
                  </div>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>
                      Fear Level at Exit: <span style={{ color: C.red, fontWeight: 800 }}>{postTradeForm.fear_level}/10</span>
                    </label>
                    <input type="range" min="1" max="10" value={postTradeForm.fear_level}
                      onChange={e => setPostTradeForm({...postTradeForm, fear_level: parseInt(e.target.value)})}
                      style={{ width: "100%", cursor: "pointer" }} />
                  </div>

                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>
                      Greed Level at Exit: <span style={{ color: C.green, fontWeight: 800 }}>{postTradeForm.greed_level}/10</span>
                    </label>
                    <input type="range" min="1" max="10" value={postTradeForm.greed_level}
                      onChange={e => setPostTradeForm({...postTradeForm, greed_level: parseInt(e.target.value)})}
                      style={{ width: "100%", cursor: "pointer" }} />
                  </div>

                  {/* Rule Violations */}
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>Any rule violations?</label>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {[
                        "Risked too much",
                        "No stop loss",
                        ...
                        <label key={violation} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 11 }}>
                          <input type="checkbox"
                            checked={postTradeForm.rule_violations.includes(violation)}
                            onChange={e => {
                              const violations = postTradeForm.rule_violations || [];
                              if (e.target.checked) {
                                setPostTradeForm({...postTradeForm, rule_violations: [...violations, violation]});
                              } else {
                                setPostTradeForm({...postTradeForm, rule_violations: violations.filter(v => v !== violation)});
                              }
                            }}
                  ...
                  <div>
                    <label style={{ fontSize: 11, color: C.muted, display: "block", marginBottom: 8, fontWeight: 600 }}>What did you learn? ⭐</label>
                    <textarea value={postTradeForm.post_trade_reflection} onChange={e => setPostTradeForm({...postTradeForm, post_trade_reflection: e.target.value})}
                      placeholder="What went well? What could improve? Any patterns noticed?"
                      style={{ width: "100%", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontSize: 12, outline: "none", boxSizing: "border-box", resize: "vertical", minHeight: 80, fontFamily: "inherit" }} />
                  </div>
                </div>

                <div style={{ display: "flex", gap: 8 }}>
                  <Btn onClick={() => setStage("trade-entry")} variant="secondary" style={{ flex: 1, padding: 12, fontSize: 13 }}>
                    ← Back
                  </Btn>
                  <Btn onClick={handlePostTradeSubmit} disabled={submitting || done || !postTradeForm.exit_reason || !postTradeForm.post_trade_reflection} 
                    style={{ flex: 1, padding: 12, fontSize: 13 }}>
                    {done ? "✓ Saved" : submitting ? "Saving..." : "✓ Complete Trade"}
                  </Btn>
                </div>
              </>
            )}
        ...
        <Card>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "#0b0d19", border: `2px dashed ${C.border}`, borderRadius: 12, padding: 32, textAlign: "center", cursor: "pointer" }}
              onClick={() => fileRef.current?.click()}
              onDragOver={e => e.preventDefault()}
              onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) { const evt = { target: { files: [f] } }; handleFile(evt); } }}>
              <div style={{ fontSize: 32, marginBottom: 10 }}>📁</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 6 }}>Drop your CSV here</div>
              <div style={{ fontSize: 12, color: C.muted, marginBottom: 16 }}>or click to browse</div>
              <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                {["Tradovate", "NinjaTrader", "Rithmic", "Amp Futures", "Generic CSV"].map(b => (
                  <span key={b} style={{ background: C.blue + "15", color: C.blue, padding: "3px 10px", borderRadius: 12, fontSize: 11 }}>{b}</span>
                ))}
              </div>
            </div>
            <input ref={fileRef} type="file" accept=".csv,.txt" onChange={handleFile} style={{ display: "none" }} />

            {csvError && <div style={{ color: C.red, fontSize: 13, background: C.red + "10", padding: "10px 14px", borderRadius: 8 }}>⚠️ {csvError}</div>}

            {csvTrades.length > 0 && (
              <>
                <div style={{ background: C.green + "10", border: `1px solid ${C.green}25`, borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 13, color: C.green, fontWeight: 700 }}>✓ {csvTrades.length} trades ready to import</span>
                  <span style={{ fontSize: 12, color: C.muted }}>Total P&L: <Pill value={csvTrades.reduce((s, t) => s + (t.pnl || 0), 0)} /></span>
                </div>
                <div style={{ overflowX: "auto", maxHeight: 240, overflowY: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    <thead style={{ position: "sticky", top: 0, background: C.panel }}>
                      <tr>{["Date", "Symbol", "Dir", "Setup", "P&L"].map(h => (
                    ...
                    <tbody>
                      {csvTrades.map((t, i) => (
                        <tr key={i} style={{ borderBottom: `1px solid ${C.border}20` }}>
                          <td style={{ padding: "7px 8px", color: C.sub, fontFamily: "monospace" }}>{t.trade_date?.slice(5)}</td>
                          <td style={{ padding: "7px 8px", color: C.text, fontFamily: "monospace" }}>{t.symbol}</td>
                          <td style={{ padding: "7px 8px", color: t.direction === "Long" ? C.green : C.red }}>{t.direction}</td>
                          <td style={{ padding: "7px 8px" }}><Tag label={t.setup_type} color={C.purple} /></td>
                          <td style={{ padding: "7px 8px" }}><Pill value={t.pnl || 0} /></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                <Btn onClick={handleCsvImport} disabled={submitting || done} style={{ width: "100%", padding: 14, fontSize: 14 }}>
                  {done ? "✓ Imported!" : submitting ? `Importing ${csvTrades.length} trades...` : `Import ${csvTrades.length} Trades →`}
                </Btn>
              </>
            )}

            <div style={{ fontSize: 11, color: C.muted, background: "#0b0d19", borderRadius: 8, padding: 12 }}>
              <div style={{ fontWeight: 700, marginBottom: 6, color: C.sub }}>Expected CSV columns (auto-detected):</div>
              <code style={{ color: C.blue }}>symbol, direction, entry_price, exit_price, pnl, position_size, date, time</code>
            </div>
          </div>
        </Card>
      )}
...
function TradeReview({ trades, setView }) {
  ...
  useEffect(() => {
    if (!lastTrade) { setLoading(false); return; }
    const fetch_ = async () => {
      try {
...
{"disciplineScore":number,"setupRating":"A+"|"A"|"B"|"C","isAplusSetup":boolean,"emotionalFlags":string[],"mistakeTags":string[],"ruleBreaks":string[],"improvement":string,"summary":string,"strengths":string[]}`,
        ...
        // 🆕 Generate dynamic coaching insights based on trade characteristics
        const generateCoachingInsight = () => {
          const pnl = lastTrade.pnl || 0;
          const isProfitable = pnl > 0; // ✓ Breakeven ($0) is NOT a win
          const isBreakeven = pnl === 0;
          const isLoss = pnl < 0;
          
          const isSmallSize = (lastTrade.position_size || 1) < 1;
          const hasNotes = lastTrade.notes && lastTrade.notes.trim().length > 0;
          const setupType = lastTrade.setup_type || "Unknown";
          const disciplineScore = lastTrade.discipline_score || 5;
          
          const riskRatio = lastTrade.take_profit && lastTrade.stop_loss && lastTrade.entry_price
            ? Math.abs((lastTrade.take_profit - lastTrade.entry_price) / (lastTrade.entry_price - lastTrade.stop_loss))
            : null;
          
          // Count how many checklist items passed
          const checklistPassed = [lastTrade.trend_aligned, lastTrade.setup_valid, lastTrade.risk_ok].filter(Boolean).length;
          const emotionalData = lastTrade.fear_level || lastTrade.greed_level;
          
          // Count rule violations (mistakes)
          const ruleViolations = [];
          if (!lastTrade.trend_aligned) ruleViolations.push("trend");
          if (!lastTrade.setup_valid) ruleViolations.push("setup");
          if (!lastTrade.risk_ok) ruleViolations.push("risk");
          if (!lastTrade.pre_trade_checklist_passed) ruleViolations.push("checklist");
          if (lastTrade.rule_violations?.length > 0) ruleViolations.push(...lastTrade.rule_violations);
          
          const hasMultipleMistakes = ruleViolations.length >= 2;
          const hasSevereMistakes = ruleViolations.length >= 3;

          let insights = [];

          // ✅ PROFITABLE TRADE INSIGHTS
          if (isProfitable) {
            if (disciplineScore >= 8 && checklistPassed >= 2) {
              if (riskRatio && riskRatio >= 2) {
                insights.push("Excellent trade—great risk-reward and strong discipline. This is the standard to maintain.");
              } else {
                insights.push("Solid winning trade with good discipline. Document what worked and replicate this setup in similar market conditions.");
              }
            } else if (disciplineScore < 7) {
              insights.push("You won despite poor discipline. Don't rely on luck—tighten your pre-trade checklist for consistent profits.");
            } else if (riskRatio && riskRatio >= 2) {
              insights.push("Great risk-reward ratio execution—maintain this discipline on future trades.");
            } else if (riskRatio && riskRatio < 1) {
              insights.push("You won this trade, but your risk-reward was unfavorable. Aim for 2:1+ on every setup.");
            } else {
              insights.push("Solid winning trade. Document what worked and replicate this setup in similar market conditions.");
            }
          }
          // ⚠️ BREAKEVEN TRADE INSIGHTS  
          else if (isBreakeven) {
            if (hasMultipleMistakes) {
              insights.push("You broke even despite multiple mistakes. Tighten your pre-trade checklist—you got lucky this time.");
            } else if (!checklistPassed) {
              insights.push("Breakeven is a wash. You skipped checklist steps—next time, validate trend, setup quality, and risk before entering.");
            } else if (riskRatio && riskRatio < 1) {
              insights.push("Breakeven with poor risk-reward. Enforce a 2:1 minimum or skip the trade entirely.");
            } else {
              insights.push("Breakeven isn't a win. Review why you entered this setup and what conditions made it go sideways.");
            }
          }
          // ❌ LOSS INSIGHTS
          else {
            if (hasSevereMistakes) {
              insights.push("This loss came from multiple discipline breakdowns. Focus on one checklist item at a time until it's automatic.");
            } else if (!checklistPassed) {
              insights.push("This loss came from skipping the pre-trade checklist. Always validate trend alignment and setup quality before entering.");
            } else if (riskRatio && riskRatio < 1) {
              insights.push("You followed your checklist but risked too much relative to potential reward. Adjust position size to enforce 2:1 risk-reward minimum.");
            } else if (disciplineScore < 5) {
              insights.push("Low discipline score triggered this loss. Slow down, validate your setup, and wait for high-conviction entries.");
            } else {
              insights.push("Review your entry timing and signal confirmation. Was this a valid setup at entry, or did you chase price action?");
            }
          }

          // 🧠 BEHAVIOR-SPECIFIC INSIGHTS
          if (!emotionalData && !hasNotes) {
            insights.push("Add mental state notes to your trades—this helps identify emotional patterns that affect decision-making.");
          } else if (emotionalData && (lastTrade.fear_level > 7 || lastTrade.greed_level > 7)) {
            insights.push("High emotional intensity detected. Practice taking smaller positions when you feel overconfident or fearful.");
          }

          // Position sizing insights (only if not already addressed)
          if (isSmallSize && isProfitable && !insights.some(s => s.includes("size"))) {
            insights.push("You won with a small position—consider increasing size slightly on A+ setups to capture better profits.");
          }

          // Default fallback
          if (insights.length === 0) {
            insights.push("Review your pre-trade checklist and post-trade reflection. Consistent journaling reveals behavioral patterns that drive performance.");
          }

          return insights[0]; // Return first (most relevant) insight
        };
        ...
        // Identify mistakes
        if (!lastTrade.trend_aligned) mistakes.push("Ignored trend");
        if (!lastTrade.setup_valid) mistakes.push("Invalid setup");
        if (!lastTrade.risk_ok) mistakes.push("Poor risk sizing");
        if (!lastTrade.pre_trade_checklist_passed) mistakes.push("Skipped checklist");
        if (lastTrade.rule_violations?.length > 0) mistakes.push(...lastTrade.rule_violations);

        // Identify strengths
        if (lastTrade.trend_aligned) strengths.push("Trend-aligned entry");
        if (lastTrade.setup_valid) strengths.push("Valid setup");
        if (lastTrade.risk_ok) strengths.push("Proper risk sizing");
        if (lastTrade.pre_trade_checklist_passed) strengths.push("Followed checklist");
        if ((lastTrade.pnl || 0) > 0) strengths.push("Profitable execution"); // ✓ Only for actual wins
        if (!strengths.length) strengths.push("Identified learning opportunity");

        setReview({
          disciplineScore: lastTrade.discipline_score || (lastTrade.pre_trade_checklist_passed ? 8 : 5),
          ...
          mistakeTags: mistakes.length > 0 ? mistakes : [],
          ruleBreaks: lastTrade.rule_violations || [],
          strengths: strengths.length > 0 ? strengths : ["Trade completed"],
          improvement: coachingInsight,
          summary: `${lastTrade.direction} trade on ${lastTrade.symbol}. ${(lastTrade.pnl || 0) > 0 ? "Profitable execution with solid setup adherence." : (lastTrade.pnl || 0) === 0 ? "Breakeven trade—review entry criteria." : "Loss trade — review entry criteria and timing."}`
        });
      }
      setLoading(false);
    };
...
// ─────────────────────────────────────────────────────────────
// CAN I TRADE — CUSTOMIZABLE CHECKLIST
// ─────────────────────────────────────────────────────────────
const CATEGORY_ICONS = { Setup: "📈", Risk: "🛡️", Mental: "🧠", Review: "📋", Custom: "⭐" };
const CATEGORY_COLORS = { Setup: C.blue, Risk: C.yellow, Mental: C.purple, Review: C.green, Custom: C.text };

function TradeChecklist({ setView }) {
  const [items, setItems] = useLocalStorage("fos_checklist", DEFAULT_CHECKLIST);
  ...
  const evaluate = () => {
    ...
    setResult(pct === 1 ? "take" : pct >= 0.7 ? "risky" : "no");
  };
  ...
  const addItem = () => {
    ...
    setItems(prev => [...prev, { id, text: newText.trim(), category: newCategory }]);
    setNewText("");
  };
  ...
  const configs = {
    take:  { label: "✅ TAKE THE TRADE",        color: C.green,  desc: "All conditions met. Execute with full size and discipline." },
    risky: { label: "⚠️ PROCEED WITH CAUTION",  color: C.yellow, desc: `${total - yesCount} condition${total - yesCount > 1 ? "s" : ""} not met. Consider reducing size by 50%.` },
    no:    { label: "❌ DO NOT TRADE",           color: C.red,    desc: "Too many conditions not met. Wait for a better setup." },
  };
  ...
  return (
    <div style={{ maxWidth: 580, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <button onClick={() => setView("dashboard")} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 13, padding: 0 }}>‹ Dashboard</button>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginTop: 8, flexWrap: "wrap", gap: 8 }}>
          <div>
            <h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 800 }}>Can I Take This Trade?</h2>
            <p style={{ color: C.muted, fontSize: 12, margin: 0 }}>Pre-trade decision engine · {items.length} checks</p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {mode === "check" && result && <Btn variant="ghost" onClick={reset} style={{ fontSize: 12, padding: "7px 14px" }}>Reset</Btn>}
            <Btn variant={mode === "edit" ? "primary" : "ghost"} onClick={() => { setMode(m => m === "check" ? "edit" : "check"); reset(); }} style={{ fontSize: 12, padding: "7px 14px" }}>
              {mode === "edit" ? "✓ Done Editing" : "✏️ Customize"}
            </Btn>
          </div>
        </div>
      </div>
      ...
      {mode === "edit" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card>
            <SectionTitle>Your Checklist Items</SectionTitle>
            {items.map((item, idx) => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderTop: idx > 0 ? `1px solid ${C.border}20` : "none" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
                  <button onClick={() => moveItem(item.id, -1)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 10, padding: "1px 4px", lineHeight: 1 }}>▲</button>
                  <button onClick={() => moveItem(item.id, 1)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 10, padding: "1px 4px", lineHeight: 1 }}>▼</button>
                </div>
                ...
                {/* Text — inline edit */}
                {editingId === item.id ? (
                  <input value={editText} onChange={e => setEditText(e.target.value)} onKeyDown={e => e.key === "Enter" && saveEdit(item.id)}
                    autoFocus
                    style={{ flex: 1, background: "#1a1d2e", border: `1px solid ${C.blue}`, borderRadius: 6, padding: "6px 10px", color: C.text, fontSize: 12, outline: "none" }} />
                ) : (
                  <span style={{ flex: 1, fontSize: 12, color: C.text, lineHeight: 1.4 }}>{item.text}</span>
                )}
                ...
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  {editingId === item.id ? (
                    <>
                      <button onClick={() => saveEdit(item.id)} style={{ background: C.green + "18", border: "none", borderRadius: 6, padding: "4px 10px", color: C.green, cursor: "pointer", fontSize: 11 }}>Save</button>
                      <button onClick={() => setEditingId(null)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 11 }}>Cancel</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => { setEditingId(item.id); setEditText(item.text); }}
                        style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "4px 8px", color: C.sub, cursor: "pointer", fontSize: 11 }}>✏️</button>
          ...
          <Card>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input value={newText} onChange={e => setNewText(e.target.value)} onKeyDown={e => e.key === "Enter" && addItem()}
                placeholder="e.g. Volume confirmed entry direction"
                style={{ width: "100%", background: "#1a1d2e", border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontSize: 13, outline: "none", boxSizing: "border-box" }} />
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: 10, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>Category</label>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {["Setup", "Risk", "Mental", "Review", "Custom"].map(cat => (
                      <button key={cat} onClick={() => setNewCategory(cat)}
                        style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${newCategory === cat ? CATEGORY_COLORS[cat] : C.border}`, background: newCategory === cat ? CATEGORY_COLORS[cat] + "20" : "transparent", color: newCategory === cat ? CATEGORY_COLORS[cat] : C.muted, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
              ...
              <Btn onClick={addItem} disabled={!newText.trim()} style={{ width: "100%" }}>+ Add Item</Btn>
            </div>
          </Card>

          <button onClick={() => { setItems(DEFAULT_CHECKLIST); reset(); }}
            style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px", color: C.muted, cursor: "pointer", fontSize: 12, textAlign: "center" }}>
      ...
      {mode === "check" && !result && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {/* Progress bar */}
          <div style={{ background: C.border, borderRadius: 4, height: 4, overflow: "hidden", marginBottom: 4 }}>
            <div style={{ height: "100%", width: `${(Object.keys(answers).length / items.length) * 100}%`, background: C.blue, borderRadius: 4, transition: "width 0.3s ease" }} />
          </div>
          <div style={{ fontSize: 11, color: C.muted, textAlign: "right", marginTop: -8 }}>{Object.keys(answers).length}/{items.length} answered</div>
          ...
          {Object.entries(grouped).map(([category, catItems]) => (
            <div key={category}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, marginTop: 4 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: CATEGORY_COLORS[category] || C.sub, textTransform: "uppercase", letterSpacing: "0.1em" }}>{category}</span>
              </div>
              {catItems.map(item => {
                const ans = answers[item.id];
                return (
                  <Card key={item.id} glow={ans === true ? C.green : ans === false ? C.red : null} style={{ padding: 16, marginBottom: 8 }}>
                    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                        <button onClick={() => setAnswers(p => ({ ...p, [item.id]: true }))}
                          style={{ padding: "7px 14px", borderRadius: 8, border: `1px solid ${ans === true ? C.green : C.border}`, background: ans === true ? C.green + "20" : "transparent", color: ans === true ? C.green : C.muted, cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
                          Yes ✓
                        </button>
                        <button onClick={() => setAnswers(p => ({ ...p, [item.id]: false }))}
                          style={{ padding: "7px 14px", borderRadius: 8, border: `1px solid ${ans === false ? C.red : C.border}`, background: ans === false ? C.red + "20" : "transparent", color: ans === false ? C.red : C.muted, cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
                          No ✕
                        </button>
                      </div>
                    </div>
          ...
          <Btn onClick={evaluate} disabled={!allAnswered} style={{ width: "100%", padding: 14, fontSize: 14 }}>
            Get My Answer → ({yesCount}/{total} passed)
          </Btn>
        </div>
      )}
      ...
      {mode === "check" && result && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card glow={configs[result].color} style={{ textAlign: "center", padding: 32 }}>
            <div style={{ fontSize: 24, fontWeight: 900, color: configs[result].color, letterSpacing: "0.04em", marginBottom: 10 }}>{configs[result].label}</div>
            <div style={{ fontSize: 13, color: C.muted, marginBottom: 20 }}>{configs[result].desc}</div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", fontSize: 13 }}>
              <span style={{ color: C.green }}>✓ {yesCount} passed</span>
              <span style={{ color: C.muted }}>·</span>
              <span style={{ color: C.red }}>✕ {total - yesCount} failed</span>
            </div>
          </Card>

          {/* Show which items failed */}
          {Object.entries(answers).filter(([,v]) => v === false).length > 0 && (
            <Card>
              <SectionTitle>Failed Items</SectionTitle>
              {items.filter(i => answers[i.id] === false).map(item => (
                <div key={item.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 0", borderTop: `1px solid ${C.border}20` }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <Btn variant="ghost" onClick={reset} style={{ flex: 1 }}>Check Again</Btn>
            {result === "take" && <Btn onClick={() => setView("entry")} style={{ flex: 1 }}>Log This Trade →</Btn>}
          </div>
        </div>
      )}
    </div>
...
function TradeLog({ trades, deleteTrade, updateTrade, setView, openTradeModal }) {
  const [filter, setFilter] = useState("all");
  const [editingId, setEditingId] = useState(null);
  
  const filtered = filter === "all" ? trades : trades.filter(t => t.result === filter || t.setup_type === filter);

  const startEdit = (trade) => {
    // Store the trade ID in sessionStorage so TradeEntry can access it
    sessionStorage.setItem('editingTradeId', trade.id);
    sessionStorage.setItem('editingTradeData', JSON.stringify(trade));
    sessionStorage.setItem('editingStage', 'pre-trade');  // 🆕 Reset to Stage 1 for full editing
    setView('entry');
  };
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        {["all", "Win", "Loss", "A+", "B"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "6px 14px", borderRadius: 20, border: `1px solid ${filter === f ? C.blue : C.border}`, background: filter === f ? C.blue + "20" : "transparent", color: filter === f ? C.blue : C.muted, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
            {f === "all" ? "All Trades" : f}
          </button>
        ))}
      </div>
      ...
      <Card style={{ padding: 0 }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id} style={{ borderBottom: `1px solid ${C.border}15` }}
                  onMouseEnter={e => e.currentTarget.style.background = "#ffffff05"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <td style={{ padding: "11px 14px", color: C.muted, fontFamily: "monospace", whiteSpace: "nowrap" }}>{t.trade_date?.slice(5)}<br /><span style={{ fontSize: 10 }}>{t.trade_time?.slice(0, 5)}</span></td>
                  <td style={{ padding: "11px 14px", color: C.text, fontFamily: "monospace", fontWeight: 700 }}>{t.symbol}</td>
                  <td style={{ padding: "11px 14px", color: t.direction === "Long" ? C.green : C.red, fontWeight: 700 }}>{t.direction === "Long" ? "↑" : "↓"}</td>
                  <td style={{ padding: "11px 14px" }}><Tag label={t.setup_type || "—"} color={C.purple} /></td>
                  ...
                  <td style={{ padding: "11px 14px" }}><Pill value={t.pnl || 0} /></td>
                  <td style={{ padding: "11px 14px", color: !t.discipline_score ? C.muted : t.discipline_score >= 8 ? C.green : t.discipline_score >= 6 ? C.yellow : C.red, fontFamily: "monospace" }}>{t.discipline_score ? `${t.discipline_score}/10` : "—"}</td>
                  <td style={{ padding: "11px 14px", display: "flex", gap: 6 }}>
                    <button onClick={() => startEdit(t)}
                      style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 14, padding: "2px 6px", borderRadius: 4 }}
...
export default function App() {
  ...
  const views = { dashboard: () => <Dashboard trades={trades} setView={setView} showSetup={showSetup && !isConfigured} setShowSetup={setShowSetup} displayName={displayName} />, entry: () => <TradeEntry addTrade={addTrade} updateTrade={updateTrade} setView={setView} trades={trades} />, review: () => <TradeReview trades={trades} setView={setView} />, checklist: () => <TradeChecklist setView={setView} />, analytics: () => <Analytics trades={trades} />, market: () => <Market />, journal: () => <TradeLog trades={trades} deleteTrade={deleteTrade} updateTrade={updateTrade} setView={setView} openTradeModal={openTradeModal} />, profile: () => <ProfileSettings profile={profile} updateProfile={updateProfile} signOut={signOut} setView={setView} /> };
