...
{stage === "pre-trade" && (
  <Card>
    ...
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {["Trending Up", "Trending Down", "Ranging", "Volatile"].map(cond => (
          <button
            key={cond}
            onClick={() => setForm(p => ({...p, market_condition: cond}))}
            style={{
              padding: "14px 12px",
              borderRadius: 8,
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <select
        value={form.entry_signal}
        onChange={e => setForm(p => ({...p, entry_signal: e.target.value}))}
      ...
      >
        ...
        <optgroup label="Price Action Patterns">
          {builtInList
            ...
            .filter(s => strategyPreferences?.builtInEnabled?.[s.value] !== false)
        ...
        {customStrategies && customStrategies.length > 0 && (
          <optgroup label="My Custom Strategies">
            {customStrategies.filter(s => s.enabled !== false).map(strat => (
              <option key={strat.id || strat.name} value={strat.name}>
                {strat.name}
              </option>
            ))}
          </optgroup>
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <input
        type="range"
        min="1" max="10"
        value={form.conviction_level}
        onChange={e => setForm(p => ({...p, conviction_level: +e.target.value}))}
    ...
    <div style={{ marginBottom: 24, padding: "12px", background: "#1a1d2e", borderRadius: 8, border: `1px solid ${C.border}` }}>
      <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
        <input
          type="checkbox"
          checked={form.is_news_event}
          onChange={e => setForm(p => ({...p, is_news_event: e.target.checked}))}
    ...
    {/* Mindset Section */}
    <div style={{ marginBottom: 24, padding: "16px", background: "#1a1d2e", borderRadius: 8, border: `1px solid ${C.border}` }}>
      <label style={{ fontSize: 13, fontWeight: 600, color: "#ffffff", display: "block", marginBottom: 12 }}>Mindset</label>
      
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: "block", marginBottom: 6, color: "#ffffff" }}>Mental State</label>
        <select 
          value={form.mental_state || ''} 
          onChange={e => setForm(p => ({...p, mental_state: e.target.value}))}
          style={{ width: "100%", padding: "12px", background: "#0f111a", border: `1px solid ${C.border}`, borderRadius: 8, color: "#ffffff" }}
        >
      ...
      <div>
        <label style={{ display: "block", marginBottom: 8, color: "#ffffff" }}>
          Emotional Intensity (1-10): <span style={{ color: C.orange }}>{form.emotional_intensity}</span>
        </label>
        <input
          type="range"
          min="1" max="10"
          value={form.emotional_intensity}
          onChange={e => setForm(p => ({...p, emotional_intensity: +e.target.value}))}
   ...
   {/* STAGE 2: EXECUTION - WITH STOP LOSS, TAKE PROFIT & RISK:REWARD */}
{stage === "trade-entry" && (
  <Card>
    <h3>2. Execution</h3>

    {/* Symbol + Trade Date + Position Size */}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
      ...
      <div>
        ...
        <input 
          ...
          onChange={e => setForm(p => ({...p, exit_time: e.target.value}))} 
          style={{ width: "100%", padding: 14, background: "#1a1d2e", color: "#e0e0e0", borderRadius: 8 }} 
        />
      </div>
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
      <div>
        ...
        <input 
          ...
          style={{ width: "100%", padding: 16, background: "#1a1d2e", color: "#ffffff", borderRadius: 8 }} 
        />
      </div>
      <div>
    ...
    <div style={{ margin: "20px 0", padding: 16, background: "#1a1d2e", borderRadius: 8, textAlign: "center" }}>
      ...
      <div style={{ fontSize: "22px", fontWeight: 700, color: C.blue }}>
        {riskReward}
      </div>
    </div>
...
// ─────────────────────────────────────────────────────────────
// CAN I TRADE — PER-USER CHECKLIST + FULL EDIT MODE
// ─────────────────────────────────────────────────────────────
...
function TradeChecklist({ setView, userId }) {
  ...
  const evaluate = () => {
    if (percentage >= 85) setResult("take");
    else if (percentage >= 65) setResult("risky");
    else setResult("no");
  };
  ...
  const configs = {
    take:  { label: "✅ TAKE THE TRADE", color: C.green, desc: `Excellent (${percentage}%)` },
    risky: { label: "⚠️ PROCEED WITH CAUTION", color: C.yellow, desc: `Moderate (${percentage}%)` },
    no:    { label: "❌ DO NOT TRADE", color: C.red, desc: `Poor (${percentage}%)` },
  };
    ...
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      ...
      {mode === "check" && result && (
        <Card glow={configs[result].color} style={{ textAlign: "center", padding: 32 }}>
          ...
          <p style={{ color: C.muted, marginTop: 12 }}>{configs[result].desc}</p>
          <Btn variant="ghost" onClick={reset} style={{ marginTop: 20 }}>Check Again</Btn>
        </Card>
      )}
